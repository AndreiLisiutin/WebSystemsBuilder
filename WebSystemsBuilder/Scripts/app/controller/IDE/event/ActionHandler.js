Ext.define('WebSystemsBuilder.controller.IDE.event.ActionHandler', {
    ...
    onEditParam:function(btn){
        ...
        if (!selectedParam){
            WebSystemsBuilder.utils.MessageBox.show('Не выбран параметр.', null, -1);
            return;
        }
        var form = win.form;
        if (!form){
            WebSystemsBuilder.utils.MessageBox.show('Форме не был передан необходимый объект.', null, -1);
            return;
        }
    ...
    onSave:function (btn) {
        ...
        var error = '';
        if (!handler.getValue()){
            WebSystemsBuilder.utils.MessageBox.show('Выберите обработчик события', null, -1);
            return;
        }
