Ext.define('WebSystemsBuilder.utils.formGenerator.GeneratorFormFactory', {
    singleton: true,
    requires: [
        'WebSystemsBuilder.utils.formGenerator.GeneratorEventsFactory'
    ],
        ...
        Ext.Ajax.request({
            ...
            success: function (objServerResponse) {
                ...
                if (jsonResp.resultCode == 0) {
                    ...
                    WebSystemsBuilder.utils.formGenerator.GeneratorEventsFactory.addEvents(window);
                    window.show();
                    newWin = window;
                } else {
    ...
    _buildControl: function (control) {
        ...
        result.items = [];
        result.dockedItems = [];
        if (result.CONTROL_TYPE_ID != ControlTypes.GridColumn
            && result.CONTROL_TYPE_ID != ControlTypes.DateColumn
            && result.CONTROL_TYPE_ID != ControlTypes.NumberColumn) {
            result.columns = [];
        }
    ...
    _initializeControl: function (window, control) {
        ...
        if (control.QUERY_MAPPINGS != null && control.QUERY_MAPPINGS.length > 0 || control.xtype == 'gridpanel') {
    ...
    _createGridpanelStore: function (control) {
        ...
        control.columns.forEach(function (column) {
            ...
            column.dataIndex = 'g_' + queryMapping.queryOutParameterID.toString() + '';
            var modelEntity = {
        ...
        store.loadSync = function (control, callback) {
            ...
            store.load({
                ...
                callback: function (records, lala, success) {
                    debugger;
                }
            });
        };
